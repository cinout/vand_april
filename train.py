...
def train(args):
    ...
    model, _, preprocess = open_clip.create_model_and_transforms(
        args.model, image_size, pretrained=args.pretrained
    )
    model.to(device)
    ...
    optimizer = torch.optim.Adam(
        list(trainable_layer.parameters()), lr=learning_rate, betas=(0.5, 0.999)
    )
    ...
    for epoch in range(epochs):
        ...
        for items in train_dataloader:
            ...
            with torch.cuda.amp.autocast():
                ...
                for layer in range(len(patch_tokens)):
                    patch_tokens[layer] /= patch_tokens[layer].norm(
                        dim=-1, keepdim=True
                    )
        ...
        if (epoch + 1) % args.save_freq == 0:
            ...
            torch.save(
                {"trainable_linearlayer": trainable_layer.state_dict()}, ckp_path
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        nargs="+",
        default=[3, 6, 9],
        help="features to be mapped into the joint embedding space",
    )
    # hyper-parameter
    ...
    parser.add_argument(
        ...
        type=int,
        default=224,
        help="the size of the images inputted into the CLIP model",
    )
    parser.add_argument(
        "--aug_rate",
        type=float,
        default=0.2,
        help="the probability of stitching images (only for mvtec)",
    )
