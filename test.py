...
def test(args):
    ...
    obj_list = test_data.get_cls_names()  # ['bottle', 'cable', ... ]
    ...
    results["gt_sp"] = []
    results["pr_sp"] = []  # prediction of sample(image)'s anomaly score
    for items in test_dataloader:
        ...
        with torch.no_grad(), torch.cuda.amp.autocast():
            image_features, patch_tokens = model.encode_image(
                image, features_list
            )  # image_features.shape: [1, 768]; patch_tokens of len==4, each shape [1, 1370, 1024]
            image_features /= image_features.norm(dim=-1, keepdim=True)
            text_features = []

            # cls_name is just one item list, e.g., ["bottle"]
            for cls in cls_name:
                text_features.append(text_prompts[cls])
            text_features = torch.stack(text_features, dim=0)  # shape: [1, 768, 2]

            # sample
            text_probs = (100.0 * image_features @ text_features[0]).softmax(
                dim=-1
            )  # shape: [1, 2]
            ...
            # visualization
            path = items["img_path"]
            cls = path[0].split("/")[-2]  # good, broken_small, ...
            filename = path[0].split("/")[-1]
            vis = cv2.cvtColor(
                cv2.resize(cv2.imread(path[0]), (img_size, img_size)), cv2.COLOR_BGR2RGB
            )  # RGB
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--save_path",
        type=str,
        default="./results/tiaoshi",
        help="path to save results",
    )
    parser.add_argument(
        "--checkpoint_path",
        type=str,
        default="./exps/vit_huge_14/model_epoch12.pth",
        help="path to save results",
    )
    parser.add_argument(
        "--config_path",
        type=str,
        default="./open_clip/model_configs/ViT-B-16.json",
        help="model configs",
    )
    # model
    ...
    parser.add_argument(
        "--pretrained", type=str, default="laion400m_e32", help="pretrained weight used"
    )
    parser.add_argument(
        "--features_list", type=int, nargs="+", default=[3, 6, 9], help="features used"
    )
    parser.add_argument(
        "--few_shot_features",
        type=int,
    ...
    parser.add_argument(
        "--mode", type=str, default="zero_shot", help="zero shot or few shot"
    )
    # few shot
    parser.add_argument(
        "--k_shot", type=int, default=10, help="e.g., 10-shot, 5-shot, 1-shot"
    )
